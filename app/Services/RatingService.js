...
module.exports = {
    ...
    JOIN school_year ON rating.sy_id = school_year.id
    JOIN subjects ON rating.subject_id = subjects.id
    JOIN students AS s ON rating.SID = s.SID`
        pool.query(_query,
    ...
    show: (id, callBack) => {
        let _query = `SELECT s.SID, CONCAT(s.Fname," ",s.Lname) AS student_name, rating.description, teachers.name, classes._class, subjects.subject, school_year.year, school_year.semester, 
        ROUND(AVG(rating1), 2) AS rating1, 
        ROUND(AVG(rating2),2) AS rating2, 
        ROUND(AVG(rating3),2) AS rating3, 
        ROUND(AVG(rating4), 2) AS rating4, 
